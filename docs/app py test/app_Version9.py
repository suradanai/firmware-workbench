...
def analyze_firmware_auto(fw_path, log_func=print):
    result = []
    ...
    # 2. วิเคราะห์ rootfs (สมมติเป็น squashfs ที่ offset 0x0240000)
    ROOTFS_OFFSET = 0x0240000
    ROOTFS_SIZE   = 0x0610000 - 0x0240000
    ...
    try:
        ...
        with open(fw_path, "rb") as f:
            ...
            with open(rootfs_bin, "wb") as fo:
                fo.write(rootfs)
        # Try unsquashfs (must have unsquashfs tool)
    ...
    # วิเคราะห์และแนะนำ patch ที่ควรทำ
    ai_patch_plan = {
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # AI Auto Analysis & Auto Patch Button
    ...
    def ai_auto_patch(self):
        ...
        def ai_worker():
            findings, ai_patch_plan = analyze_firmware_auto(self.fw_path)
            for line in findings:
                self.info(line)
            # วางแผนการ patch อัตโนมัติตามที่ AI วิเคราะห์
            patch_steps = []
            fw_base = os.path.basename(self.fw_path)
            fw_in = self.fw_path
            fw_prev = fw_in
            # 1. patch boot delay ถ้าจำเป็น
            if ai_patch_plan["patch_bootdelay"]:
                ...
                fw_prev = fw_out
            # 2. patch shell serial ถ้าจำเป็น
