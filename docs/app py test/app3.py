import sys, subprocess, threading, hashlib, struct
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QPushButton, 
    QTextEdit, QFileDialog, QLabel, QComboBox, QHBoxLayout, QMessageBox, QTabWidget
)
from PySide6.QtCore import Qt

# --- เพิ่มข้อมูล partition offset ของเฟิร์มแวร์คุณ ---
PARTITION_OFFSETS = [
...
def get_partition_details(fw_path):
    # อ่านไฟล์และแสดง partition table + magic
...
def get_entropy(fw_path, sample_size=4096, samples=16):
    # คำนวณ entropy เฉพาะตัวอย่าง (เร็ว)
    import math
    res = []
    with open(fw_path, "rb") as f:
...
class MainWindow(QMainWindow):
    ...
    def show_fw_info(self):
        # ตรวจสอบข้อมูล firmware แล้วแสดงใน Info tab
        if not self.fw_path:
            QMessageBox.warning(self, "ยังไม่ได้เลือกไฟล์", "กรุณาเลือกไฟล์ firmware ก่อน")
            return
