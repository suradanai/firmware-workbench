...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QFileDialog, QLabel, QComboBox, QHBoxLayout, QMessageBox, QTabWidget, QLineEdit, QSpinBox
)
...
# --- AIWorker สำหรับ QThread ที่ปลอดภัยกับ GUI ---
...
class AIWorker(QObject):
    ...
    def run(self):
        try:
            findings = analyze_firmware_detailed(
                self.fw_path,
                self.offset,
                self.size,
            ...
            self.error.emit(f"❌ วิเคราะห์ผิดพลาด: {e}\n{tb}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- Entropy sampling options ---
        entropy_layout = QHBoxLayout()
        entropy_layout.addWidget(QLabel("Entropy: จำนวน sample"))
        self.entropy_samples_spin = QSpinBox()
        self.entropy_samples_spin.setRange(1, 32)
        self.entropy_samples_spin.setValue(4)
        entropy_layout.addWidget(self.entropy_samples_spin)
        entropy_layout.addWidget(QLabel("ขนาด sample (KB)"))
        self.entropy_size_spin = QSpinBox()
        self.entropy_size_spin.setRange(1, 1024)
        self.entropy_size_spin.setValue(64)
        entropy_layout.addWidget(self.entropy_size_spin)
        entropy_layout.addStretch()
        main_layout.addLayout(entropy_layout)
        # ------------------------------
        ...
        ai_layout.addWidget(self.btn_ai_analyze)

        self.btn_export_report = QPushButton("Export ผลวิเคราะห์ AI เป็นไฟล์")
        self.btn_export_report.clicked.connect(self.export_analysis_report)
        ai_layout.addWidget(self.btn_export_report)

        self.btn_show_ai_findings = QPushButton("สรุปข้อควรระวังจาก AI")
        self.btn_show_ai_findings.clicked.connect(self.show_ai_findings)
        ai_layout.addWidget(self.btn_show_ai_findings)
    ...
    def ai_analyze_only(self):
        if self.ai_running:
            QMessageBox.information(self, "กำลังวิเคราะห์", "โปรดรอให้การวิเคราะห์เสร็จสิ้นก่อน")
            return
        if not self.fw_path:
            QMessageBox.warning(self, "ยังไม่ได้เลือกไฟล์", "กรุณาเลือกไฟล์ firmware ก่อน")
            return
        ...
        # --- ใช้ QThread + AIWorker ปลอดภัยกับ GUI ---
        self.ai_thread = QThread()
        self.ai_worker = AIWorker(self.fw_path, offset, size)
        self.ai_worker.moveToThread(self.ai_thread)
        self.ai_thread.started.connect(self.ai_worker.run)
        self.ai_worker.log.connect(self.log)
        self.ai_worker.finished.connect(self.ai_ai_done)
        self.ai_worker.error.connect(self.ai_ai_error)
        self.ai_worker.finished.connect(self.ai_thread.quit)
        self.ai_worker.error.connect(self.ai_thread.quit)
        self.ai_thread.finished.connect(self.ai_thread.deleteLater)
        self.ai_thread.start()

    def ai_ai_done(self, findings):
    ...
    def export_analysis_report(self):
        if not self.analysis_result:
            QMessageBox.warning(self, "ยังไม่มีผลวิเคราะห์", "โปรดวิเคราะห์ firmware ก่อน")
            return
        fname, _ = QFileDialog.getSaveFileName(self, "บันทึกผลวิเคราะห์", "", "Text Files (*.txt);;All Files (*)")
        if fname:
            with open(fname, "w", encoding="utf-8") as f:
                f.write("\n".join(self.analysis_result))
            QMessageBox.information(self, "บันทึกสำเร็จ", f"บันทึกที่ {fname}")

    def show_ai_findings(self):
        if not self.analysis_result:
            QMessageBox.warning(self, "ยังไม่มีผลวิเคราะห์", "กรุณาวิเคราะห์ firmware ก่อน")
            return
        text = "\n".join(self.analysis_result)
        # Highlight security warning
        warnings = [line for line in self.analysis_result if any(
            kw in line.lower() for kw in ["telnet", "ftp", "shell debug", "ไม่มีรหัสผ่าน", "ไม่มี shell", "ผิดปกติ", "ไม่พบ", "error", "entropy ต่ำ", "อาจผิดปกติ"]
        )]
        if warnings:
            warn_txt = "\n".join(warnings)
            QMessageBox.critical(self, "ข้อควรระวัง/ช่องโหว่", warn_txt)
        else:
            QMessageBox.information(self, "AI Findings", text)

    def patch_root_password(self):
        if self.pw_running:
            QMessageBox.information(self, "กำลังแก้ไขรหัสผ่าน", "โปรดรอให้การแก้ไขเสร็จสิ้นก่อน")
            return
        if not self.fw_path:
            QMessageBox.warning(self, "ยังไม่ได้เลือกไฟล์", "กรุณาเลือกไฟล์ firmware ก่อน")
            return
        ...
        # --- ใช้ AI findings ช่วย auto-fill และแจ้งเตือน ---
        if self.analysis_result:
            for finding in self.analysis_result:
                if "user root ไม่มีรหัสผ่าน" in finding or "user root ถูกล็อก" in finding:
                    QMessageBox.warning(self, "ข้อควรระวัง", "user root เดิมไม่มีรหัสผ่านหรือถูกล็อก ควรตั้งรหัสใหม่ให้ปลอดภัย")
        # ---------------------------------------------------

        self.pw_running = True
        self.btn_patch_rootpw.setEnabled(False)
        ...
        def pw_worker():
            ...
            try:
                ...
                try:
                    ...
                    QTimer.singleShot(0, lambda: QMessageBox.critical(self, "ผิดพลาด", f"แตก rootfs ไม่สำเร็จ: {e}"))
                    return
                ...
                if not os.path.exists(shadow_path):
                    ...
                    QTimer.singleShot(0, lambda: QMessageBox.critical(self, "ผิดพลาด", "ไม่พบไฟล์ /etc/shadow ใน rootfs"))
                    return
                ...
                if not found:
                    ...
                    QTimer.singleShot(0, lambda: QMessageBox.critical(self, "ผิดพลาด", "ไม่พบ user root ใน /etc/shadow"))
                    return
                ...
                if not mksquashfs_cmd(unsquashfs_dir, new_rootfs_bin, timeout=90):
                    ...
                    QTimer.singleShot(0, lambda: QMessageBox.critical(self, "ผิดพลาด", "สร้าง squashfs ไม่สำเร็จ (ลองทั้ง xz/gzip แล้ว)"))
                    return
                ...
                QTimer.singleShot(0, lambda: QMessageBox.information(self, "สำเร็จ", f"Patch root password ใน shadow สำเร็จ!\nไฟล์: {out_file}"))
                self.patched_fw = out_file
            except Exception as e:
                import traceback
                tb = traceback.format_exc()
                self.log(f"❌ Patch root password ผิดพลาด: {e}\n{tb}")
            finally:
                def enable_pw_btn():
                    self.btn_patch_rootpw.setEnabled(True)
                    self.pw_running = False
                QTimer.singleShot(0, enable_pw_btn)
        threading.Thread(target=pw_worker, daemon=True).start()

    def verify_firmware(self):
        if self.verify_running:
            QMessageBox.information(self, "กำลังตรวจสอบ", "โปรดรอให้การตรวจสอบเสร็จสิ้นก่อน")
            return
        ...
        def verify_worker():
            ...
            try:
                ...
                self.log(f"Entropy (ตัวอย่าง): {get_entropy(fw, sample_size=sample_size, samples=samples)}")
            ...
            def enable_verify_btn():
                self.btn_verify.setEnabled(True)
                self.verify_running = False
            QTimer.singleShot(0, enable_verify_btn)
